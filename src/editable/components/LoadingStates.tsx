...
export function PageLoadingState({ label = 'Loading page', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto w-full max-w-7xl rounded-[2rem] border border-white/10 bg-[rgba(10,18,34,0.78)] px-4 py-12 text-white shadow-[0_22px_60px_rgba(0,0,0,0.2)] sm:px-6 lg:px-8', className)} aria-live="polite" aria-busy="true">
      <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#35d7ff]">{label}</p>
      <PulseBlock className="mt-5 h-12 w-3/4 max-w-3xl" />
      <PulseBlock className="mt-4 h-5 w-2/3 max-w-2xl" />
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {[0, 1, 2].map((item) => (
          <div key={item} className="rounded-[2rem] border border-white/10 p-5">
  ...
  return (
    <div className={cn('grid gap-5 sm:grid-cols-2 lg:grid-cols-3', className)} aria-live="polite" aria-busy="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="rounded-[1.75rem] border border-white/10 bg-[rgba(10,18,34,0.78)] p-4 text-white">
...
export function DetailLoadingState({ label = 'Loading detail', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto grid w-full max-w-6xl gap-8 rounded-[2rem] border border-white/10 bg-[rgba(10,18,34,0.78)] px-4 py-12 text-white shadow-[0_22px_60px_rgba(0,0,0,0.2)] lg:grid-cols-[0.8fr_1.2fr]', className)} aria-live="polite" aria-busy="true">
